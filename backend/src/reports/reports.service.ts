...
export class ReportsService {
  ...
  async generatePdf(projectId: number): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      ...
      doc
        .fontSize(12)
        .text(`Total Vulnerabilities: ${lastScan.vulnerabilities.length}`);

      const counts = lastScan.vulnerabilities.reduce(
        (acc, v) => {
      ...
      lastScan.vulnerabilities.forEach((v, index) => {
        doc
          .fontSize(14)
          .text(`${index + 1}. ${v.packageName} (${v.severity.toUpperCase()})`);
