...
export class RemediationService {
  ...
  async fixVulnerability(
    ...
    if (!user || !user.accessToken) {
      throw new ForbiddenException(
        'GitHub access token missing. Please reconnect your GitHub account.',
      );
    }

    const packageName = vulnerability.packageName;
    const tempDir = path.join(
      this.configService.get<string>('scan.tempDir') || './tmp',
      `fix-${vulnerabilityId}-${Date.now()}`,
    );
    await fs.ensureDir(tempDir);

    try {
      this.logger.log(
        `Starting remediation for ${packageName} in project ${project.name}`,
      );
      ...
      if (project.lockfilePath && project.lockfilePath !== '.') {
        if (path.extname(project.lockfilePath)) {
          ...
          lockFilePath = path
            .join(project.lockfilePath, lockFilename)
            .replace(/\\/g, '/');
        }
      }

      const packageJsonPath = path
        .join(path.dirname(lockFilePath), 'package.json')
        .replace(/\\/g, '/');
      ...
      try {
        ...
        lockfileContent = await this.githubService.getFileContent(
          project.repositoryUrl,
          lockFilePath,
          project.branch,
          user.accessToken,
      ...
      if (packageManager === PackageManager.NPM) {
        ...
        throw new UnprocessableEntityException(
          `Auto-fix currently only supports 'npm'. Project is using '${packageManager}'.`,
        );
      }
      ...
      try {
        ...
        throw new UnprocessableEntityException(
          `Dependency upgrade failed. The package might be incompatible or requires manual intervention.`,
        );
      }

      // 4. Read updated files
      const newPackageJson = await fs.readFile(
        path.join(tempDir, 'package.json'),
        'utf-8',
      );
      const newLockfile = await fs.readFile(
      ...
      if (
        newPackageJson === packageJsonContent! &&
        newLockfile === lockfileContent!
      ) {
        throw new UnprocessableEntityException(
          'Upgrade command completed but no files were changed. You might already be on the latest version.',
        );
      }
      ...
      throw new InternalServerErrorException(
        error.message || 'Auto-fix failed due to an unexpected error',
      );
    } finally {
  ...
  private handleGithubError(error: any, action: string): never {
    if (error.status === 401 || error.message.includes('Bad credentials')) {
      throw new ForbiddenException(
        `GitHub Authentication failed during ${action}. Check your token.`,
      );
    }
    if (error.status === 404) {
      throw new NotFoundException(
        `Resource not found during ${action}. Check repository access.`,
      );
    }
    if (error.status === 409) {
      throw new ConflictException(
        `Conflict during ${action}. A branch/PR might already exist.`,
      );
    }
    if (error.status === 422) {
      throw new UnprocessableEntityException(
        `Validation failed during ${action}: ${error.message}`,
      );
    }
    throw new BadGatewayException(
      `GitHub API failed during ${action}: ${error.message}`,
    );
  }
