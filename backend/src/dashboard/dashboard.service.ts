...
export class DashboardService {
  ...
  async getStats(userId: number) {
    ...
    const totalVulnerabilities = { critical: 0, high: 0, moderate: 0, low: 0 };
    ...
    for (const project of projects) {
      ...
      if (lastScan) {
        ...
        lastScan.vulnerabilities.forEach((v) => {
          if (!v.whitelisted) {
            // Should check whitelist? Assuming boolean flag on vuln for now or derived
        ...
        projectStats.push({
          ...
          criticalCount: lastScan.vulnerabilities.filter(
            (v) => v.severity === 'critical' && !v.whitelisted,
          ).length,
          highCount: lastScan.vulnerabilities.filter(
            (v) => v.severity === 'high' && !v.whitelisted,
          ).length,
        });
      } else {
        projectStats.push({
    ...
    const averageScore =
      scoredProjectsCount > 0 ? totalScore / scoredProjectsCount : 0;
