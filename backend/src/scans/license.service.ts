...
import { VulnerabilitySeverity, VulnerabilityType, Vulnerability } from './vulnerability.entity';
...
export class LicenseService {
    ...
    async checkLicenses(dependencies: Record<string, string>): Promise<Partial<Vulnerability>[]> {
                ...
                try {
                    ...
                    if (license && this.isRisky(license)) {
                        findings.push({
                            ...
                            severity: license.includes('AGPL') || license.includes('GPL') ? VulnerabilitySeverity.HIGH : VulnerabilitySeverity.MODERATE,
                    ...
                    this.logger.warn(`Failed to check license for ${pkg}: ${e.message}`);
                }
            }));
        }
    ...
    private async fetchLicense(packageName: string): Promise<string | null> {
        try {
            ...
            const response = await axios.get(`https://registry.npmjs.org/${packageName}/latest`, { timeout: 5000 });
