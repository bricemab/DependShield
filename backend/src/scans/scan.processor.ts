...
export class ScanProcessor {
  ...
  async handleScan(job: Job<{ scanId: number }>) {
    ...
    try {
      ...
      try {
        ...
        this.logger.warn(
          `Failed to update GitHub status: ${statusError.message}`,
        );
      ...
      if (scan.project.lockfilePath && scan.project.lockfilePath !== '.') {
        if (path.extname(scan.project.lockfilePath)) {
          ...
          targetPath = path
            .join(scan.project.lockfilePath, lockFilename)
            .replace(/\\/g, '/');
        }
      }
      ...
      await fs.writeFile(
        path.join(scanDir, savedLockfilename),
        lockfileContent,
      );
      this.logger.log(
        `Lockfile ${savedLockfilename} fetched and saved to ${scanDir}`,
      );
      await this.updateProgress(scan, 20, 'Lockfile downloaded');

      // Also fetch package.json just in case
      try {
        const packageJsonPath = targetPath.replace(
          savedLockfilename,
          'package.json',
        );
        ...
        this.logger.warn(
          `Could not fetch package.json, continuing without it: ${e.message}`,
        );
      }
      ...
      scan.dependencyGraph = await this.generateDependencyGraph(
        scanDir,
        scan.project.packageManager,
      );
      ...
      // Run full audit
      const fullAuditResult = await this.runAudit(
        scanDir,
        scan.project.packageManager,
        false,
      );
      const fullVulnerabilities = this.parseAuditResult(
        fullAuditResult,
        scan.project.packageManager,
      );

      // Run production-only audit
      const prodAuditResult = await this.runAudit(
        scanDir,
        scan.project.packageManager,
        true,
      );
      const prodVulnerabilities = this.parseAuditResult(
        prodAuditResult,
        scan.project.packageManager,
      );
      ...
      const vulnerabilities: Partial<Vulnerability>[] = fullVulnerabilities.map(
        (v) => {
      ...
      // Run Docker Scan
      const dockerVulns = await this.scanDocker(
        scan.project.repositoryUrl,
        scan.project.branch,
        user.accessToken,
      );
      ...
      const whitelistRules = await this.whitelistService.findAllByProject(
        scan.project.id,
      );

      // Enrich vulnerabilities with EPSS scores
      this.logger.log(
        `Fetching EPSS scores for ${vulnerabilities.length} vulnerabilities...`,
      );
      ...
      if (cves.length > 0) {
        try {
          ...
          this.logger.log(
            `Found ${ghsaIds.length} GHSA IDs and ${realCves.length} CVEs`,
          );
          ...
          if (ghsaIds.length > 0) {
            ...
            ghsaToCveMap = await this.githubService.getCvesFromGhsas(
              ghsaIds,
              user.accessToken,
            );
            this.logger.log(
              `Successfully converted ${ghsaToCveMap.size} GHSA IDs to CVEs`,
            );
          }
          ...
          if (finalCves.length > 0) {
            this.logger.log(
              `Fetching EPSS scores for ${finalCves.length} CVEs...`,
            );
            const epssScores =
              await this.epssService.getCachedOrFetch(finalCves);
            this.logger.log(
              `Successfully fetched ${epssScores.size} EPSS scores`,
            );
      ...
      try {
        ...
        if (await fs.pathExists(packageJsonPath)) {
          ...
          if (pkg.dependencies) {
            const licenseVulns = await this.licenseService.checkLicenses(
              pkg.dependencies,
            );
            this.logger.log(
              `Found ${licenseVulns.length} restrictive licenses.`,
            );
      ...
      if (
        ...
        try {
          ...
          if (await fs.pathExists(packageJsonPath)) {
            const pkg = await fs.readJson(packageJsonPath);
            if (pkg.dependencies) {
              const findings = await this.supplyChainService.checkTyposquatting(
                pkg.dependencies,
              );
              if (findings.length > 0) {
                this.logger.warn(
                  `Found ${findings.length} potential typosquatting attacks!`,
                );
          ...
          this.logger.error(`Supply Chain scan failed: ${e.message}`);
        }
      } else {
        this.logger.log(
      ...
      if (scan.project.emailEnabled && scan.project.user.email) {
        await this.notificationsService.sendScanResultEmail(
          scan.project.user.email,
          scan,
        );
      }
      ...
      if (scan.commitSha) {
        const statusState =
          activeVulnerabilities.length > 0 ? 'failure' : 'success';
        const description =
      ...
      if (scan.commitSha) {
        try {
          const userForError = await this.usersService.findOne(
            scan.project.user.id,
          );
          ...
          this.logger.warn(
            `Failed to update GitHub status on error: ${e.message}`,
          );
        }
      }
    } finally {
      ...
      try {
        ...
        this.logger.error(
          `Failed to cleanup scan directory for scan ${scanId}`,
          cleanupError,
        );
      }
    }
  ...
  private async updateProgress(scan: Scan, progress: number, message?: string) {
    ...
    await this.scansRepository.save(scan);
    this.logger.log(
      `Scan ${scan.id} progress: ${progress}%${message ? ` - ${message}` : ''}`,
    );
  }
  ...
  private async runAudit(
    ...
    switch (packageManager) {
      ...
      case 'yarn':
        ...
        if (prodOnly) {
          this.logger.warn(
            'Yarn audit does not support production-only filtering reliably via JSON.',
          );
        }
        break;
      case 'pnpm':
            ...
            for (const viaItem of vulnData.via) {
              ...
              vulnerabilities.push({
                ...
                severity: this.mapSeverity(
                  viaItem.severity || vulnData.severity,
                ),
  ...
  private async scanDocker(
    ...
    try {
      const dockerfileContent = await this.githubService.getFileContent(
        repoUrl,
        'Dockerfile',
        branch,
        token,
      ...
      if (
        / FROM\s+ [\w\-\/\.]+(:latest)?\s+/i.test(dockerfileContent) ||
        !/FROM\s+[\w\-\/\.]+:/i.test(dockerfileContent)
      ) {
        if (
          / FROM\s + [\w\-\/\.]+:latest/i.test(dockerfileContent) ||
          /FROM\s+[\w\-\/\.]+\s+$/m.test(dockerfileContent)
        ) {
          vulns.push({
            ...
            description:
              'Using the "latest" tag leads to non-reproducible builds and potential breakage.',
      ...
      if (!/USER\s+\w+/i.test(dockerfileContent)) {
        vulns.push({
          ...
          description:
            'No USER instruction found. Running as root is a security risk.',
          type: VulnerabilityType.DOCKER,
          isDevDependency: false,
        });
      }
      ...
      if (/ADD\s+/i.test(dockerfileContent)) {
        vulns.push({
          ...
          description:
            'ADD has features (remote URL fetching, tar extraction) that can be dangerous. Use COPY for local files.',
          type: VulnerabilityType.DOCKER,
          isDevDependency: false,
        });
      }
      ...
      this.logger.log(
        `No Dockerfile found or could not be scanned: ${e.message}`,
      );
    }
    return vulns;
  }
  ...
  private async scanSecrets(
    ...
    const vulns: Partial<Vulnerability>[] = [];
    this.logger.log(
      `[SecretScan] Starting advanced secret scan for ${repoUrl} on branch ${branch}...`,
    );
    ...
    const secretPatterns = [
      ...
      },
      {
        ...
        regex: /["']?[A-Za-z0-9\/+=]{40}["']?/,
        contextRegex: /aws_secret_access_key|aws_secret_key|secret_key/i,
        severity: VulnerabilitySeverity.CRITICAL,
      },
      {
      ...
      }, // Less critical but still bad practice
      {
        name: 'Google API Key',
    ...
    try {
      ...
      this.logger.log(
        `[SecretScan] Found ${filesToScan.length} potential files. Scanning up to ${MAX_FILES_TO_SCAN}...`,
      );
      ...
      if (deps) {
        for (const [name, dep] of Object.entries(deps as any)) {
          ...
          if ((dep as any).dependencies) {
            for (const [subName, subDep] of Object.entries(
              (dep as any).dependencies as any,
            )) {
