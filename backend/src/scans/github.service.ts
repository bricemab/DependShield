...
export class GithubService {
  ...
  async getFileContent(
    ...
    try {
      ...
      this.logger.debug(
        `Fetching file ${path} from ${owner}/${repo} on branch ${branch}`,
      );
      ...
      this.logger.error(
        `Failed to fetch file ${path} from ${repoUrl}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async getCommitSha(
    ...
    try {
      ...
      this.logger.error(
        `Failed to get commit SHA for ${owner}/${repo} branch ${branch}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async updateCommitStatus(
    ...
    try {
      ...
      this.logger.error(
        `Failed to update commit status for ${sha}: ${error.message}`,
      );
  ...
  async getCveFromGhsa(ghsaId: string, token?: string): Promise<string | null> {
    try {
      ...
      if (response.data?.data?.securityAdvisory?.identifiers) {
        ...
        if (cveIdentifier) {
          this.logger.debug(
            `Found CVE ${cveIdentifier.value} for GHSA ${ghsaId}`,
          );
      ...
      this.logger.warn(
        `Failed to fetch CVE for GHSA ${ghsaId}: ${error.message}`,
      );
      return null;
    }
  }
  ...
  async getCvesFromGhsas(
    ...
    for (let i = 0; i < ghsaIds.length; i += batchSize) {
      ...
      const promises = batch.map((ghsaId) =>
        this.getCveFromGhsa(ghsaId, token),
      );
  ...
  async createBranch(
    ...
    try {
      ...
      this.logger.log(
        `Created branch ${newBranch} from ${baseBranch} on ${owner}/${repo}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to create branch ${newBranch}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async pushChanges(
    ...
    try {
      ...
      this.logger.error(
        `Failed to push changes to ${branch}: ${error.message}`,
      );
      throw error;
    }
  }
