...
describe('ScansService', () => {
    ...
    describe('triggerScan', () => {
        it('should create a scan and add it to the queue', async () => {
            ...
            expect(mockScansRepository.create).toHaveBeenCalledWith({ projectId, status: ScanStatus.PENDING });
            expect(mockScansRepository.save).toHaveBeenCalledWith(scan);
            expect(mockScansQueue.add).toHaveBeenCalledWith('scan', { scanId: scan.id });
        });
        ...
        it('should throw BadRequestException if rate limit exceeded', async () => {
            ...
            await expect(service.triggerScan(projectId, userId)).rejects.toThrow(BadRequestException);
        });
