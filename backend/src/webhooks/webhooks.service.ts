...
export class WebhooksService {
    ...
    async findAllByProject(projectId: number, userId: number): Promise<Webhook[]> {
    ...
    async create(projectId: number, userId: number, createDto: CreateWebhookDto): Promise<Webhook> {
        const user = await this.usersService.findOne(userId);
        if (user.plan !== 'PRO' && user.plan !== 'ENTERPRISE') {
            throw new ForbiddenException('Webhooks are available for PRO and ENTERPRISE plans only.');
        }
    ...
    async triggerWebhook(project: any, scan: Scan, event: WebhookEvent) {
        const webhooks = await this.webhooksRepository.find({ where: { projectId: project.id, isActive: true } });
