...
import { GitProvider, GitRepository, GitBranch, GitLockfile } from '../interfaces/git-provider.interface';
...
export class GithubProvider implements GitProvider {
    ...
    async detectLockfiles(accessToken: string, owner: string, repo: string, branch: string): Promise<GitLockfile[]> {
        if (!accessToken) {
            throw new ForbiddenException('GitHub access token not found');
        }
