...
export class EpssService {
  ...
  constructor(
    ...
    this.apiUrl =
      this.configService.get<string>('epss.apiUrl') ||
  ...
  private async fetchBatch(cves: string[]): Promise<EpssScoreDto[]> {
    try {
      ...
      if (response.data && response.data.data) {
        this.logger.log(
          `Successfully fetched ${response.data.data.length} EPSS scores`,
        );
  ...
  private async cacheScore(score: EpssScoreDto): Promise<void> {
    try {
      ...
      this.logger.error(
        `Redis cache write error for ${score.cve}: ${error.message}`,
      );
